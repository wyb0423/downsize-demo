...
AI 国家也会自动降级，但比玩家简化很多：

| 维度 | 玩家 | AI |
|------|------|-----|
| 触发方式 | JE 脉冲（可选每周/每月） | 全局 `on_half_yearly_pulse_country`（每半年一次） |
| UI 控制 | 有（启用/禁用 + 频率切换） | 无（始终静默运行） |
| 人手阈值 | < 50% | < 20%（更严格，只处理极端情况） |
| 现金阈值 | < 25% | < 5% |
| 盈利阈值 | ≤ 0 | < -200（每周严重亏损） |
| 严重程度 | 严重 + 中度两档 | 只有严重档 |
| 冷却期 | 6 个月 | 6 个月 |
...
这样设计的原因：AI 经济本身比较脆弱，降级太激进可能适得其反。只在建筑"彻底废了"时才出手。
