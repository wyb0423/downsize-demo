...
| `bg_extraction` | 矿场、伐木场、渔场、油田、橡胶园等 | ✅ 新增：资源建筑依赖地块禀赋，即使暂时亏损也不降级 |
| 产权比例 = 0 | 完全政府持有的建筑等级 | ✅ 新增：只降私有等级，保护玩家可手动操作的等级 |

**② 触发条件 — 同时满足下面四条才会被降级：**

| 条件 | 代码 | 阈值 | 含义 |
|------|------|------|------|
| 有私有产权 | `private_ownership_fraction > 0` | 大于 0% | ✅ 新增：只处理有私有/公司持有的建筑 |
...
每个降级档位还有一个隐含条件：`private_ownership_fraction ≥ 门槛`。门槛 = 降级量 / 该档最低等级，确保即使在最坏情况下，也有足够的私有等级可扣。

| 降级档 | 严重门槛 | 中度门槛 | 含义 |
|--------|---------|---------|------|
| 100+ 级 | 12% | 8% | 100级建筑至少要有12/8级是私有的 |
| 51-100 级 | 16% | 10% | 51级建筑至少要有8/5级是私有的 |
| 26-50 级 | 20% | 12% | 26级建筑至少要有5/3级是私有的 |
| 11-25 级 | 28% | 19% | 11级建筑至少要有3/2级是私有的 |
| 1-10 级 | 50% (保守) | 50% (保守) | 低等级建筑只有私有过半才动 |

> **举例**：一栋 50 级纺织厂，30 级私有（60%）+ 20 级政府（40%），严重亏损 → 判定降 5 级（门槛 20%，实际 60% ≥ 20% ✓）→ 只扣私有部分，政府 20 级纹丝不动。
